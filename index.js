...
bot.onText(/\/intro/, async (message) => {
    try {
        await bot.sendPhoto(message.chat.id, "https://i.postimg.cc/XJtnc0kF/bot-intro-1.png", {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "Pkey", callback_data: "pkey" }]
                ]
            }
        })
    } catch (error) {
...
bot.on("callback_query", async (callback_query) => {
    try {
        ...
        const sentMessage = await bot.sendMessage(callback_query.message.chat.id, "Generating...")
        await bot.deleteMessage(chat_id, callback_query.message.message_id)
                ...
                Jimp.read(dataObj.link)
                    .then(image => {
                        ...
                        return Jimp.read("https://i.postimg.cc/sgpmQKHK/logo-tns.png")
                            ...
                            .then(imageBuffer => {
                                bot.sendPhoto(chat_id, imageBuffer, {
                                    reply_markup: {
                                        inline_keyboard: [
